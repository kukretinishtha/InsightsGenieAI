...
st.set_page_config(page_title="Data Pipeline Visualization", layout="wide")

st.title("📊 InsightGenie - Data Pipeline")
st.markdown("**Real-time stock data from Bronze → Silver → Gold layers**")
...
with tab1:
    ...
    bronze_data = {
        "symbol": ["INFY", "TCS", "WIPRO", "LT", "HCL", "RELIANCE", "HDFC", "SBIN"],
        "price": [1850.50, 3750.75, 625.25, 2450.00, 1850.00, 2550.75, 2700.50, 550.25],
        "volume": [2500000, 1800000, 3200000, 1500000, 2200000, 2100000, 1600000, 3500000],
        "source": ["yfinance"] * 8,
...
with tab4:
    ...
    with col1:
        ...
        top_stocks = df_gold.nlargest(3, 'total_volume')[['symbol', 'avg_price', 'trend']]
    ...
    with col2:
        ...
        avg_price = df_gold['avg_price'].mean()
        avg_volatility = df_gold['volatility'].mean()
    ...
    # Comparison chart
    fig = px.bar(df_gold, x="symbol", y=["avg_price", "volatility"],
                 title="Price vs Volatility Comparison",
                 barmode="group")
    st.plotly_chart(fig, use_container_width=True)
    
    # Price range by symbol
    df_gold['price_range'] = df_gold['max_price'] - df_gold['min_price']
    fig = px.waterfall(df_gold, x="symbol", y="price_range",
                       title="Daily Price Range by Symbol")
    st.plotly_chart(fig, use_container_width=True)
